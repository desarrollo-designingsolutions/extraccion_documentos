...
from .schemas import RespuestaLLM  # Asume que RespuestaLLM está definido en schemas
...
async def call_llm(request: Request, context: str) -> Dict:
    ...
    system_content = """
        ...
        - Para "numero_factura", usa el literal detectado (alfanumérico permitido con guiones/puntos). No completa prefijos ni consecutivos faltantes.
...
async def responder_pregunta_multiple(request: Request, input_data: PreguntaMultipleInput, db: AsyncSession = Depends(get_db)):
    ...
    async def process_nit(nit: str) -> Dict[str, Any]:
        async with sem:
            ...
            try:
                count_result = await db.execute(query_debug, {"nit": nit})
            ...
            if count == 0:
                ...
                try:
                    archivo_result = await db.execute(query_archivo, {"nit": nit})
                    if archivo_result.fetchone() is None:
                        raise HTTPException(status_code=404, detail=f"NIT {nit} no encontrado")
                except Exception:
                    logger.exception("Error comprobando NIT")
                    raise HTTPException(status_code=500, detail="Error en DB")

            # Fetch chunks
            results = await fetch_chunks_by_nit(db, embedding, nit)
            if not results:
                raise HTTPException(status_code=404, detail=f"No chunks encontrados para NIT {nit}")
            ...
            try:
                ...
                logger.exception("Error durante reranking")
                raise HTTPException(status_code=500, detail="Error durante reranking")
            ...
            if not chunks_finales:
                raise HTTPException(status_code=400, detail=f"Chunks no relevantes para NIT {nit}")
    ...
    tasks = [process_nit(nit) for nit in input_data.nits]
